...
# Optionally, try to import your local model wrapper so that we can perform an explicit check.
try:
...
def get_embedding_matrix(model):
    # Unwrap if the model is a wrapper (e.g. LocalHuggingFaceModel)
    if hasattr(model, 'model'):
        return get_embedding_matrix(model.model)
    # Alternatively, if you want an explicit check:
    if LocalHuggingFaceModel is not None and isinstance(model, LocalHuggingFaceModel):
        return get_embedding_matrix(model.model)
    # Now, if the model is a Hugging Face model, use its getter.
    if isinstance(model, PreTrainedModel):
        emb = model.get_input_embeddings()
        if emb is None:
            raise ValueError("The Hugging Face model does not expose input embeddings via get_input_embeddings()")
        return emb.weight
    else:
...
def get_embeddings(model, input_ids):
    ...
    if hasattr(model, 'model'):
        return get_embeddings(model.model, input_ids)
    if LocalHuggingFaceModel is not None and isinstance(model, LocalHuggingFaceModel):
        return get_embeddings(model.model, input_ids)
    # If the model is a Hugging Face model, use its input embeddings.
    if isinstance(model, PreTrainedModel):
        emb = model.get_input_embeddings()
        if emb is None:
            raise ValueError("The Hugging Face model does not expose input embeddings via get_input_embeddings()")
