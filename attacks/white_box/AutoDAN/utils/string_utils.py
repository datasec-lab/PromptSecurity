...
class HFChatAutoDANSuffixManager:
    """AutoDAN prompt formatter backed by the target tokenizer's chat template."""

    def __init__(self, *, chat_formatter, instruction, adv_string):
        self.chat_formatter = chat_formatter
        self.instruction = instruction
        self.adv_string = adv_string

    def render_candidate(self, adv_string=None):
        if adv_string is not None:
            self.adv_string = adv_string
        return self.adv_string.replace('[REPLACE]', self.instruction.lower())

    def get_prompt(self, adv_string=None):
        user_content = self.render_candidate(adv_string)
        return self.chat_formatter(user_content, add_generation_prompt=True)
