...
def get_losses(*, model, tokenizer, input_ids, control_slice, 
             target_slice,
             test_controls=None, return_ids=False, batch_size=512):
...
def forward(*, model, input_ids, attention_mask, 
    ...
    for i in range(0, input_ids.shape[0], batch_size):
        ...
        # logits.append(model(input_ids=batch_input_ids, attention_mask=batch_attention_mask).logits)
        losses.append(target_loss(model(input_ids=batch_input_ids, attention_mask=batch_attention_mask).logits, 
                    batch_input_ids, 
                    target_slice)
                     )
