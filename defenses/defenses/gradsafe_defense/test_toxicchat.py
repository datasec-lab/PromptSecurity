...
from defenses.GradSafe.grad_safe import GradSafeDefense
from grad_safe import SafeGradDefendedModel


def cos_sim_toxic(target_model, df, gradient_norms_compare, minus_row, minus_col):
...
if __name__ == "__main__":
    for model_id in ["meta-llama/Llama-2-7b-chat-hf"]:
        ...
        gradient_norms_compare, minus_row_cos, minus_col_cos = defended_model.find_critical_parameters()
        df = pd.read_csv('./data/toxic-chat/toxic-chat_annotation_test.csv')
        auprc, f1 = cos_sim_toxic(target_model, df, gradient_norms_compare, minus_row_cos, minus_col_cos)